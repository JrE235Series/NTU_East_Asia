...
class AnimatedButton:
    ...
    def draw(self, surface):
        ...
        if self.clickable:
            ...
            if self.current_state == "hover":
                pygame.draw.rect(
                    button_surface,
                    (255, 255, 255, 100),  # half-transparent white
            ...
            if self.correct:
                pygame.draw.rect(
                    button_surface,
                    (80, 200, 120, 128),  # half-transparent white
                    button_surface.get_rect(),
                    border_radius=12       # adjust radius as needed
                )
            elif self.wrong:
                pygame.draw.rect(
                    button_surface,
                    (220, 80, 80, 128),  # half-transparent white
                    button_surface.get_rect(),
                    border_radius=12       # adjust radius as needed
                )
    ...
    def update_ans(self,correct,wrong):
        self.clickable = False
        if (correct) :self.correct = True       
        elif (wrong) : self.wrong = True